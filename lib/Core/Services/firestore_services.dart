...
class FireStoreService {
  ...
  Future<void>addTodoData(
      String collectionName, Map<String, dynamic> todoData) async {
    await _fireStore.collection(collectionName).doc('todo_data').set(todoData);
  }
  Future<void> addNewTodoData(String collectionName, Map<String, dynamic> todoData) async {
    await _fireStore.collection(collectionName).add(todoData);
  }
  Future<List<Map<String, dynamic>>> getAllTodos() async {
    final querySnapshot = await _fireStore.collection('todos').get();
    return querySnapshot.docs.map((doc) {
      final data = doc.data();
      data['doc_id'] = doc.id; // optional: add doc ID
      return data;
    }).toList();
  }
